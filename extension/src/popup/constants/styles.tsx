export const FONT_FAMILY = '"Muli", sans-serif';

export const COLOR_PALETTE = {
  white: "#fff",
  offWhite: "#fefefe",
  background: "#F0F2F6",
  inputBackground: "rgba(210, 216, 229, 0.4)",
  text: "#060E1A",
  secondaryText: "#748098",
  lightText: "#4B4E54",
  primary: "#FBB44A",
  primaryMuted: "#615BCE",
  primaryFaded: "#FBB44A",
  secondary: "#654CF7",
  error: "#F42703",
  errorFaded: "#FFBFBF",
  grey: "#99A2B4",
  greyFaded: "#FBB44A",
  greyDark: "#7B869D",
  warning: "#E4792C",
  warningHigh: "#E44D2C",
  warningHighFaded: "#F8CFCC",
  warningFaded: "#F8E4CC",
  primaryGradient: "linear-gradient(180deg, #6f42c1 0%, #6610f2 100%)",
  darkPrimaryGradient: "linear-gradient(180deg, #FBB44A 0%, #FBB44A 100%)",
  menuGradient: "linear-gradient(180deg, #6f42c1 0%, #6610f2 100%)",
};

export const Z_INDEXES = {
  body: 0,
  loader: -1,
  nav: 10,
};

export const ANIMATION_TIMES = {
  fast: "0.15s",
  medium: "0.3s",
  slow: "0.5s",
};

export const FONT_WEIGHT = {
  light: "300",
  normal: "400",
  bold: "600",
};

export const ROUNDED_CORNERS = "1.25rem";
